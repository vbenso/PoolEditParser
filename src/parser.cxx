...
int getRealSize(void *object)
{
    ...
    case 0:
    {
        ...
        return sizeof(WorkingSet) + workingSet->objects * sizeof(ObjectReference) + workingSet->macros * sizeof(MacroReference) + workingSet->languageCodes * sizeof(LanguageCode);
    }
    case 1:
    ...
    case 4:
        // special case!
        {
            ...
            return sizeof(SoftKeyMask) + softKeyMask->objects * sizeof(unsigned short) + softKeyMask->macros * sizeof(MacroReference);
        }
    ...
    case 8:
    {
        ...
        return sizeof(InputString) + inputString->length * sizeof(unsigned char) + *inputStringMacros(inputString) * sizeof(MacroReference) + 2;
    }
    ...
    case 10:
    {
        ...
        return sizeof(InputList) + inputList->numberOfListItems * sizeof(unsigned short) + inputList->macros * sizeof(MacroReference);
    }

    case 11:
    {
        OutputString *outputString = (OutputString *)object;
        return sizeof(OutputString) + outputString->length * sizeof(unsigned char) + *outputStringMacros(outputString) * sizeof(MacroReference) + 1;
    }
    ...
    case 16:
    {
        ...
        return sizeof(Polygon) + polygon->macros * sizeof(MacroReference) + polygon->numberOfPoints * sizeof(Point);
    }
    case 17:
    ...
    case 20:
    {
        ...
        return sizeof(PictureGraphic) + pictureGraphic->macros * sizeof(MacroReference) + pictureGraphic->rawDataLength;
    }
    ...
    case 26:
    {
        ...
        return sizeof(InputAttributes) + inputAttributes->length + *inputAttributesMacros(inputAttributes) * sizeof(PictureGraphic) + 1;
    }
    ...
    if (role == ROLE_NONE)
    {
        ...
        case 4:
        {
            *object = realloc(*object, getRealSize(*object) + 2);
            SoftKeyMask *softKeyMask = (SoftKeyMask *)*object;
            unsigned char *ptr = ((unsigned char *)softKeyMask) + sizeof(SoftKeyMask) + softKeyMask->objects * sizeof(unsigned short);
            softKeyMask->objects++;
        ...
        case 10:
        {
            *object = realloc(*object, getRealSize(*object) + 2);
            InputList *inputList = (InputList *)*object;
            unsigned char *ptr = ((unsigned char *)inputList) + sizeof(InputList) + inputList->numberOfListItems * sizeof(unsigned short);
...
{
    uint8_t type[2];            // Magic number, must be 'BM'
    uint32_t size;              // File size in bytes
    uint16_t reserved1;         // Must be 0
    uint16_t reserved2;         // Must be 0
    uint32_t offset;            // Offset to image data, usually 54 for 8-bit bitmaps
    uint32_t dib_header_size;   // Size of DIB header, usually 40
    int32_t width;              // Image width in pixels
    int32_t height;             // Image height in pixels
    uint16_t color_planes;      // Number of color planes, must be 1
    uint16_t bits_per_pixel;    // Bits per pixel, must be 8 for 256-color images
    ...
    uint32_t num_colors;        // Number of colors in palette, must be 256 for 8-bit images
    uint32_t important_colors;  // Number of important colors, 0 = all colors are important
} BMP_Header;
...
void WriteRLEtoBMP(RLE_Element *rleData, int width, int height, const char *filename)
{
    int imageSize = width * height;
    const int NUM_COLORS = 232;
    ...
    BMP_Header header = {
        .type = {'B', 'M'},
        .size = BMP_HEADER_SIZE + NUM_COLORS * sizeof(RGBQuad) + width * height,
        .reserved1 = 0,
        .reserved2 = 0,
        .offset = BMP_HEADER_SIZE + NUM_COLORS * sizeof(RGBQuad),
        .dib_header_size = 40,
        .width = width,
        .height = height,
        .color_planes = 1,
    ...
    2 (Green)	00,99,00,FF
    3 (Teal)	00,99,99,FF
    4 (Maroon)	99,00,00,FF
    5 (Purple)	99,00,99,FF
    6 (Olive)	99,99,00,FF
    7 (Silver)	CC,CC,CC,FF
    8 (Grey)	99,99,99,FF
    9 (Blue)	00,00,FF,FF
    10 (Lime)	00,FF,00,FF
    11 (Cyan)	00,FF,FF,FF
    12 (Red)	FF,00,00,FF
    13 (Magenta)FF,00,FF,FF
    14 (Yellow)	FF,FF,00,FF
    15 (Navy)	00,00,99,FF
    */
    ...
    free(img_mirror);
    if (all_counts != width * height)
    {
        printf("counts:%d size:%d\n", all_counts, width * height);
    }
...
void addPictureData(void **object)
{
    ...
    *object = realloc(*object, getRealSize(*object));
    ...
    free(data);
    /*if(picture->objectId==647) {
        exit(-1);
    }*/
}

#define INIT_OBJECT(oType, object)                     \
    oType *object = (oType *)calloc(sizeof(oType), 1); \
    object->objectId = id;                             \
    object->type = type
